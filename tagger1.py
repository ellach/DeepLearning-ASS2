...
class Solver(object):
    ...
    def epoch(self, dataset, optimize):
        ...
        for epoch in range(config.NUM_EPOCHS):
            ...
            for i in range(0, len(dataset)-config.BATCH_SIZE, config.BATCH_SIZE):
                ...
                x_batch = [[utils1.getIdOfWord(w) for w in a] for a,b in batch]
                y_batch = [utils1.getIdOfTag(b) for a,b in batch]
    ...
    def prediction(self, testDataset, type):
        ...
        for i in range(0, len(testDataset)-config.BATCH_SIZE, config.BATCH_SIZE):       
            ...
            x_batch = [[utils1.getIdOfWord(w) for w in a] for a in batch]
            x = Variable(torch.LongTensor(x_batch))
            ...
            for j in range(config.BATCH_SIZE):
                ...
                input = utils1.getWordOfId(y_hat_index) +"   "+utils1.getTagOfId(y_hat_index)
                predOutput.append(input)

        test = open('test1.'+type,'w')           
        test.write('\n'.join(predOutput)) 
        test.close()
...
if __name__ == "__main__":
 ...
 solver.epoch(utils1.windowTrain, True)

 print('Start validation ...')
 solver.epoch(utils1.windowDev,False)
  
 print('Start prediction ... ') 
 solver.prediction(utils1.windowTest, sys.argv[4])
